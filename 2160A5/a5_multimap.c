 ...
 *
 * PURPOSE: To implement the multimap using void pointers
...
struct MULTIMAP {
  ...
  // fields for the function pointers
  Compare compare_keys_func;
  Compare compare_vals_func;
};

// Helper functions
static int find_key_pos(void *key, KeyAndValues *keys, int num_keys, Multimap *mm);
static int insert_key_alphabetically(KeyAndValues *keys, int keys_length, void *key, Multimap *mm);
static int insert_value_by_num(KeyAndValues *key, ValueNode *node, Multimap *mm);
...
static int validate_multimap(Multimap *mm)
{
  ...
  for (int i = 0; i < mm->num_keys; i++) {
    ...
    if (i > 0) {
      ...
      assert(mm->compare_keys_func(mm->keys[i-1].key, mm->keys[i].key) < 0);
    }
    ...
    while (NULL != curr) {
      ...
      if (prev != NULL) {
        // ordering
        assert(mm->compare_vals_func(&curr->value, &prev->value) >= 0);
      }
      prev = curr;
      curr = curr->next;
    }
...
Multimap *mm_create(int max_keys, Compare compare_keys, Compare compare_values)
{
  ...
  if (max_keys >= 0 && compare_keys != NULL && compare_values != NULL) {
    ...
    if (NULL != mm) {
      ...
      if (NULL == mm->keys) {
        ...
        // function pointers for compare_keys and compare_values
        mm->compare_keys_func = compare_keys;
        mm->compare_vals_func = compare_values;
      }
    }
  }
...
int mm_insert_value(Multimap *mm, void *key, int value_num, void *value_data)
{
  ...
  if (NULL != mm && NULL != key && NULL != value_data) {
    pos = find_key_pos(key, mm->keys, mm->num_keys, mm);
    if (pos < 0 && mm->num_keys < mm->max_keys) {

      pos = insert_key_alphabetically(mm->keys, mm->num_keys, key, mm);
    ...
    if (pos >= 0) {
      ...
      if (NULL != node) {
        ...
        result = insert_value_by_num(&mm->keys[pos], node, mm);
      }
...
static int find_key_pos(void *key, KeyAndValues *keys, int num_keys, Multimap *mm)
{
  ...
  while (start <= end && pos < 0) {
    ...
    comp = mm->compare_keys_func(key, keys[mid].key);
    if (comp < 0) {
      end = mid - 1;
    } else if (comp > 0) {
      start = mid + 1;
...
static int insert_key_alphabetically(KeyAndValues *keys, int keys_length, void *key, Multimap *mm) {
  ...
  while (pos < keys_length && mm->compare_keys_func(key, keys[pos].key) >= 0) {
    pos++;
  }
...
static int insert_value_by_num(KeyAndValues *key, ValueNode *node, Multimap *mm) {
  ...
  while (NULL != curr && mm->compare_vals_func(&curr->value, &node->value) < 0) {
    prev = curr;
    curr = curr->next;
  }
